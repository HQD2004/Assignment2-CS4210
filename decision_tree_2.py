...
for ds in dataSets:
    ...
    X = encoder.fit_transform([row[:4] for row in dbTraining])

    #Transform the original categorical training classes to numbers and add to the vector Y.
    #For instance Yes = 1 and No = 2, Y = [1, 1, 2, 2, ...]
    #--> add your Python code here
    for row in dbTraining:
        if row[4] == 'Yes': Y.append(1)
        else: Y.append(0)
    ...
    for i in range (10):
       ...
       #--> add your Python code here
        XTest = encoder.fit_transform([row[:4] for row in dbTest])
        for data in dbTest:
           #Transform the features of the test instances to numbers following the same strategy done during training,
           #and then use the decision tree to make the class prediction. For instance: class_predicted = clf.predict([[3, 1, 2, 1]])[0]
           #where [0] is used to get an integer as the predicted class label so that you can compare it with the true label
           #--> add your Python code here
            class_predicted = clf.predict([XTest[dbTest.index(data)]])[0]
           #Compare the prediction with the true label (located at data[4]) of the test instance to start calculating the accuracy.
           #--> add your Python code here
            intTR = -1
            if data[4] == 'Yes': intTR = 1
            else: intTR = 0

            if class_predicted == intTR: correct += 1
            else: wrong += 1
